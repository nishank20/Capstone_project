...
def listener():
    ...
    msg = rospy.wait_for_message("/zed_node/point_cloud/cloud_registered", PointCloud2)
    pc_callback(msg)
    msg2 = rospy.wait_for_message("/zed_node/depth/depth_registered", Image)
    combine_image_depthimage_pointcloud(msg2)


def combine_image_depthimage_pointcloud(pointcloud):
    # Reading and visualizing the point cloud
    cloud = o3d.io.read_point_cloud("dataset/pointcloud.ply")
    o3d.visualization.draw_geometries([cloud])


if __name__ == '__main__':
    listener()
    combine_image_depthimage_pointcloud()