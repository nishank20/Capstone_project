...
class GetData():
    def __init__(self, data_dir, object_name, unit_deg=5):
        ...
        #rospy.Subscriber("/zed_node/point_cloud/cloud_registered", PointCloud2, self.pointcloud_callback_10) #recent
        #rospy.Subscriber("/zed_node/point_cloud/cloud_registered", PointCloud2, self.pointcloud_callback_3)

        rospy.Subscriber("/zed_node/depth/depth_registered", Image, self.image_callback_01)
    ...
    def image_callback_01(self, depth_image_data):
        depth_image = self.bridge.imgmsg_to_cv2(depth_image_data, depth_image_data.encoding)
        # depth_array = np.array(depth_image, dtype=np.float32)
        # print(len(depth_array))
        # cv2.normalize(depth_array, depth_array, 0, 1, cv2.NORM_MINMAX)
        # cv_image1 = depth_array * 2 ** 16
        cv2.imwrite("dataset/image" + str(self.i) +".jpg", depth_image);
        # cv2.waitKey(3);
        # print("pointcloud data 3 written! # {}".format(self.i));
        self.flag["pcl_3"] = True;

        # print("This line is executed")
        # img = open3d.geometry.Image(depth_array);
        # open3d.io.write_image("dataset/image" + str(self.i) +".jpg", img)
        # self.flag["pcl_3"] = True
